...
func TestSharding(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		os.Remove("master3.wal")
		os.Remove("slave3.wal")
		os.Remove("master3.ckp")
		os.Remove("slave3.ckp")
	})
	...
	m3 := NewStorage(mux, "master3", true)
	m3.Run()

	s3 := NewStorage(mux, "slave3", false)
	s3.Run()

	m3.AddReplica(url, "slave3")

	r.addShard(Shard{"master3", []string{"slave3"}})
	...
	sendDelete(t, url, point2)
	sendDelete(t, url, point4)

	time.Sleep(1 * time.Second)

	for i := 1; i < 100; i++ {
		collection := sendSelect(t, url, [2]float64{-100, -100}, [2]float64{100, 100})
		assert.Equal(t, 0, len(collection.Features))
	}
}
